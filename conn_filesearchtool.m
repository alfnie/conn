...
if nargin<1 || ischar(varargin{1}),
    ...
    if params.reduced, h.frame=conn_menu('frame',[1,0,0,1,1]*M);
    else h.frame=conn_menu('frame',[1,0,0,1,1]*M);
    end
    ...
    if h.reduced
        h.filename=conn_menu('edit',[1,.05,.90,.9,.10]*M,'',params.filename);
    ...
    h.find_state=0;
    h.titles_folder=conn_menu('text',[1,.10,.170,.20,.035]*M,'','path : '); set(h.titles_folder,'horizontalalignment','left');
    h.folder=conn_menu('edit',[1,.3,.170,.6,.040]*M,'',params.folder,'<HTML>Current folder<br/> - Type a full path to change to a different folder</HTML>');
    ...
    h.titles_filter=conn_menu('text',[1,.10,.120,.20,.035]*M,'','files : '); set(h.titles_filter,'horizontalalignment','left');
    h.filter=conn_menu('edit',[1,.30,.120,.6,.040]*M,'',params.filter,'<HTML>File name filter (standard format)<br/> - Show only files with names matching any of these patterns<br/> - Use ";" to define multiple filters</HTML>');
    h.titles_regexp=conn_menu('text',[1,.10,.070,.20,.035]*M,'','filter : '); set(h.titles_regexp,'horizontalalignment','left');
    h.regexp=conn_menu('edit',[1,.30,.070,.6,.040]*M,'',params.regexp,'<HTML>Additional full-path filename filter (regexp format)<br/> - Show only files with full-path filename matching this pattern<br/> - See <i>help regexp</i> for additional information');
    if ~isempty(params.localcopy)&&~h.reduced,  
        ...
        h.titles_localcopy=[];
        h.localcopy=conn_menu('popup',[1,.05,.27,.45,.040]*M,'',{'imports selected files','imports copy of selected files'},['<HTML>Controls behavior of ''',h.strbutton,''' button:<br/> - <i>import selected files</i> : (default) selected files will be imported into your CONN project directly from their original locations/folders<br/> - <i>import copy of selected files</i> : selected files will be first copied to your project conn_*/data/BIDS folder and then imported into your CONN project <br/>(e.g. use this when importing data from read-only folders if the files need to be further modified or processed)</HTML>']); set(h.localcopy,'value',params.localcopy);
    else 
        h.titles_localcopy=[];
        h.localcopy=[];
    end
