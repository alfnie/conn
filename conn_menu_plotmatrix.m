...
if ischar(x)
    ...
    if ~SERIOUSNESS, cmap=sqrt([.6 .6 .2;.8 .8 .8]); cshape='o';
    ...
    if SERIOUSNESS<2
        for n=1:abs(nr)+0,
            ...
            conn_menu_plotmatrix(.6*rand(np).*(.5+.25*reshape(conn_hanning(prod(np)),np)),'dogray',tnv,'colormap',cmap,'colormapcdata',ceil(size(cmap,1)*rand(np)),'shape',cshape(randi(numel(cshape))),'parent',h,varargin{4:end});
