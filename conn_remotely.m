...
%
%    conn remotely restart     : re-starts CONN from the "client" computer after a dropped connection (this will 
%                                not start a new CONN server process but instead will attempt to re-connect to an 
%                                existing one)
%
...
%
%   step 1) in the "host/server" computer, run the Matlab command "conn server" to manually launch a CONN server 
%           process. From the output printed by this command, note the first occurrence of a keyphrase of the form 
...
%
% CONNECTION
% conn_remotely on                                            : starts remote CONN server and connect to it (equivalent to running "conn remotely" but without GUI)
% conn_remotely off                                           : disconnects from remote CONN server (equivalent to closing GUI when using "conn remotely" to start CONN server)
% conn_remotely offandon                                      : restarts communication with remote CONN server after dropped connection (equivalent to running "conn remotely restart" but without GUI)
%
% REMOTE EXECUTION
% conn_remotely cmd                                           : interactive command-line execution in remote CONN server
% [...]=conn_remotely('cmd',...)                              : single-command execution in remote CONN server                                                      : e.g. y=conn_remotely('cmd','fft',1:4);
%                                                                                                                                                                   : e.g. conn_remotely('cmd','run','/usr/myself/mybatch01.m')
%
% REMOTE FILE COPY
% conn_remotely('push',localfile,remotefolder)                : copies local file 'localfile' into remote folder 'remotefolder'                                     : e.g. conn_remotely push /data/file1.nii /usr/myself/data/
% conn_remotely('folderpush',localfolder,remotefolder)        : copies the local folder 'localfolder' (and all of its contents) into remote folder 'remotefolder'   : e.g. conn_remotely push /data /usr/myself
% conn_remotely('pull',remotefile,localfolder)                : copies remote file 'localfile' into local folder 'localfolder'                                      : e.g. conn_remotely pull /usr/myself/data/file1.nii /data/ 
% conn_remotely('folderpull',remotefolder,localfolder)        : copies the remote folder 'remotefolder' (and all of its contents) into local folder 'localfolder'   : e.g. conn_remotely pull /usr/myself/data /
% conn_remotely('push',localfile,remotefile)                  : copies local file 'localfile' to remote storage and rename it as 'remotefile'                       : e.g. conn_remotely push /data/file1.nii /usr/myself/data/file1.nii
% conn_remotely('pull',remotefile,localfile)                  : copies remote file 'remotefile' to local storage and rename it as 'localfile'                       : e.g. conn_remotely pull /usr/myself/data/file1.nii /data/file1.nii 
%
% UTILITIES:
% When connected to a remote CONN server and running locally any conn_* function: most CONN functions that take files or directories as input arguments will accept filepaths of the form: '/CONNSERVER/[filepath]' to refer to a file/folder [filepath] in the remote server storage. 
%    e.g. conn_mesh_display /CONNSERVER/usr/myself/data/file1.nii  % will display locally the remote file file1.nii 
%    e.g. conn_cache pull /CONNSERVER/usr/myself/data/file1.nii    % will create a local-cache copy of the remote file file1.nii
%    e.g. conn_display /CONNSERVER/usr/myself/results/SPM.mat      % will display locally the remote 2nd-level results
%    e.g. conn_fileutils mkdir /CONNSERVER/usr/myself/newfolder    % will create a new remote directory (equivalent to: conn_remotely cmd mkdir /usr/myself/newfolder)
% When connected to a remote CONN server and running locally CONN_HPC functions: CONN will always use the remote-server HPC options to submit jobs instead of the local HPC options
%    e.g. conn('submit','disp','hello world')                      % will have the remote-server use its own HPC options (e.g. slurm profile) to submit a job to the remote network where the disp('hello world') function will be executed
%    e.g. conn('submit','run','/usr/myself/mybatch01.m')           % will have the remote-server use its own HPC options (e.g. slurm profile) to submit a job to the remote network where the mybatch01 script will be executed
%    e.g. conn_jobmanager('getprofile')                            % will list the remote-server default HPC profile (just the same as str=conn_remotely('cmd','conn_jobmanager','getprofile'))
%
