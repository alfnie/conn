...
% www.conn-toolbox.org/resources/cluster-configuration for additional HPC/cluster installation instructions. 
% While optional, setting up CONN's cluster/HPC configuration in your "host" computer allows CONN_REMOTELY 
% to also submit jobs to your cluster remotely (jobs submitted from the "client" computer which will be
% run in the local network of the "host" computer). 
%
...
%
% note on implementation: when conn_remotely starts, the "client" computer will attempt to connect using ssh to the "host"
% computer and it will establish a secure tunnel for communications. The host computer will then use its SERVERcmd profile
% in HPC/cluster configuration settings to launch a new Matlab session in a "server" computer, which may be the same as the
% "host" computer or a different computer within the same network (e.g. if using default HPC settings this session will be 
...
% wait for the client's connection. Once the connection is established, the "host" computer will only act as a gateway for
% communications between the "client" and the "server" computers. The "server" computer will wait for instructions from 
% the "client", while the "client" runs CONN normally (typically for anything that relates to GUI-interaction), querying
% the "server" when needed (e.g. when needing to load data from remote files), or requesting the "server" to run longer or 
% more complex steps when appropriate (e.g. when preprocessing the data or running any analysis steps that requires 
% significant interaction with the data). When the CONN gui is closed in the "client" computer, the "server" will exit 
...
%
% note: if the "host" computer is NOT SSH-accessible (e.g. another computer in your home/office network), the following 
...
%
%   step 1) in the "host/server" computer, run the Matlab command "conn server" to manually launch a CONN server process
