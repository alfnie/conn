...
if ~isempty(thr2), 
    if isequal(thr2,'equalsize'), temp=sort(b2(~isnan(b2)),'descend'); thr2=temp(nnz(ROIdata1{1}>0))-eps; end
    if isnan(thr2)||isequal(thr2,'globalmask'), thr2=0.80*mean(b2(b2>mean(b2(~isnan(b2)&b2~=0))/8)); end
    ROIdata2={b2>thr2}; 
    ROInames2={'mask2'};
