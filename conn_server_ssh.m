            ...
            while ntries>0
                if startnewserver
                    ...
                    if ~ispc||~isfield(params.info,'windowscmbugfixed')||params.info.windowscmbugfixed
                        ...
                        [ok,msg]=system(sprintf('start "Step 2/2: Requesting a new Matlab session" /WAIT echo Requesting a new Matlab session in %s. This may take a few minutes. Please wait ^&^& cmd /c %s %s "%s" ^> %s 2^>^&1', params.info.login_ip, params.options.cmd_ssh, params.info.login_ip, regexprep(sprintf(params.info.CONNcmd,tstr),'"','\\"'),tfilename));
