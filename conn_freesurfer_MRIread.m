function mri = conn_freesurfer_MRIread(fstring,headeronly)
...
[fspec fstem fmt] = conn_freesurfer_MRIfspec(fstring);
if(isempty(fspec))
...
switch(fmt)
  case {'mgh','mgz'}
  [mri.vol, M, mr_parms, volsz] = conn_freesurfer_load_mgh(fspec,[],[],headeronly);
  if(isempty(M))
...
%------- analyze -------------------------------------   
 case {'img'}
  ...
  M = conn_freesurfer_vox2ras_1to0(hdr.vox2ras);
  mri.analyzehdr = hdr;
%------- nifti nii -------------------------------------   
 case {'nii','nii.gz'}
...
mri.vox2ras1 = conn_freesurfer_vox2ras_0to1(M); 
...
mri.tkrvox2ras = conn_freesurfer_vox2ras_tkreg(mri.volsize,mri.volres);
