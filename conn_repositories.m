...
if isempty(varargin), % other parameters: names of selected datasets
    ...
    datastr=cellfun(@(a,b)sprintf('<HTML>%s (%s) <small>%s</small></HTML>',upper(a),regexp(b,'n\s*=\s*\d+','match','once'),b),{repofiles.name},{repofiles.description},'uni',0);
    s = listdlg('PromptString','Select dataset(s) from the 1000 Functional Connectomes Project to download','ListSize',[800 400],...
        'SelectionMode','multiple',...
        'ListString',conn_menu_formathtml(datastr),...
        'InitialValue',1);
