...
steps={'default_mni','default_mnifield','default_mnidirectfield','default_ss','default_ssfield','default_ssnl',...
    ...
    'structural_label_as_subjectspace', ...
    'structural_load', ...
    'structural_load_from_original', ...
    'structural_load_from_mnispace', ...
    'structural_load_from_subjectspace', ...
    ...
    'functional_load_from_minprep', ...
    'functional_load_from_realigned', ...
...
steps_names={'<HTML><b>DEFAULT PREPROCESSING PIPELINE</b> for volume-based analyses (direct normalization to MNI-space)</HTML>','<HTML><b>PREPROCESSING PIPELINE</b> for volume-based analyses (indirect normalization to MNI-space) when FieldMaps are available</HTML>','<HTML><b>PREPROCESSING PIPELINE</b> for volume-based analyses (direct normalization to MNI-space) when FieldMaps are available</HTML>','<HTML><b>PREPROCESSING PIPELINE</b> for surface-based analyses (in subject-space)</HTML>','<HTML><b>PREPROCESSING PIPELINE</b> for surface-based analyses (in subject-space) when FieldMaps are available</HTML>','<HTML><b>PREPROCESSING PIPELINE</b> for surface-based analyses (in subject-space) using nonlinear coregistration</HTML>',...
    ...
    'structural Label current structural files as "subject-space structural images"', ...
    ...
    'structural Load structural data from "subject-space structural images" dataset', ...
    ...
    'functional Label current functional files as "minimally-preprocessed functional data"', ...
    ...
    'functional Load functional data from "minimally-preprocessed functional data" dataset', ...
...
steps_names=conn_menu_formathtml(steps_names);
...
steps_descr={{'INPUT: structural&functional volumes','OUTPUT (all in MNI-space): skull-stripped normalized structural volume, Grey/White/CSF normalized masks, realigned slice-time corrected normalized smoothed functional volumes, subject movement ''realignment'' and ''scrubbing'' 1st-level covariate'},{'INPUT: structural&functional&FieldMap volumes',  'OUTPUT (all in MNI-space): skull-stripped normalized structural volume, Grey/White/CSF normalized masks, realigned&unwarp slice-time corrected normalized smoothed functional volumes, subject movement ''realignment'' and ''scrubbing'' 1st-level covariate'},{'INPUT: structural&functional&FieldMap volumes',  'OUTPUT (all in MNI-space): skull-stripped normalized structural volume, Grey/White/CSF normalized masks, realigned&unwarp slice-time corrected normalized smoothed functional volumes, subject movement ''realignment'' and ''scrubbing'' 1st-level covariate'},{'INPUT: structural&functional volumes','OUTPUT (all in subject-space): skull-stripped structural volume, Grey/White/CSF masks, realigned slice-time corrected coregistered functional volumes, subject movement ''realignment'' and ''scrubbing'' 1st-level covariate'},{'INPUT: structural&functional&VDM volumes','OUTPUT (all in subject-space): skull-stripped structural volume, Grey/White/CSF masks, realigned&unwarp slice-time corrected coregistered functional volumes, subject movement ''realignment'' and ''scrubbing'' 1st-level covariate'},{'INPUT: structural&functional volumes','OUTPUT (all in subject-space): skull-stripped structural volume, Grey/White/CSF masks, realigned slice-time corrected coregistered functional volumes, subject movement ''realignment'' and ''scrubbing'' 1st-level covariate'},...
    ...
    {'INPUT: structural volume','OUTPUT: structural volume (same files re-oriented, not resliced)'}, {'INPUT: structural volume','OUTPUT: structural volume (resliced)'}, {'INPUT: structural volume','OUTPUT: skull-stripped structural volume, Grey/White/CSF masks (in same space as structural)'},...
    ...
    {'INPUT: structural volumes','OUTPUT: none ("subject-space structural images" secondary dataset will point to current version of structural volumes)'}, ...
    ...
    {'INPUT: other imaging volumes (in secondary dataset)','OUTPUT: structural volumes (current structural volumes will point to same files as "subject-space structural images" secondary dataset)'}, ...
    ...
    {'INPUT: functional volumes','OUTPUT: none ("minimally-preprocessed data" secondary dataset will point to current version of functional volumes)'}, ...
    {'INPUT: functional volumes','OUTPUT: none ("realigned data" secondary dataset will point to current version of functional volumes)'}, ...
    ...
    {'INPUT: other imaging volumes (in secondary dataset)','OUTPUT: functional volumes (current functional volumes will point to same files as "minimally-preprocessed data" secondary dataset)'}, ...
    {'INPUT: other imaging volumes (in secondary dataset)','OUTPUT: functional volumes (current functional volumes will point to same files as "realigned data" secondary dataset)'}, ...
...
steps_combinedpipelines={...
    {'functional_label_as_original','structural_label_as_original','functional_centerruns','structural_center','functional_realign&unwarp','functional_label_as_realigned','functional_slicetime','functional_art','functional_label_as_minprep','functional_coregister_affine_reslice','functional_label_as_subjectspace','structural_label_as_subjectspace','functional_load_from_minprep','functional_segment&normalize_direct','functional_label_as_mnispace','structural_segment&normalize','structural_label_as_mnispace','functional_smooth','functional_label_as_smoothed'},...
...
for n1=1:2:numel(options)-1,
    switch(lower(options{n1}))
        ...
        case 'roi_names',
            roi_names=options{n1+1};
        case 'roi_dimensions',
            roi_dimensions=options{n1+1};
        case 'roi_deriv',
            roi_deriv=options{n1+1};
        case 'roi_filter',
            roi_filter=options{n1+1};
        case 'roi_detrend',
...
if dogui&&any(ismember(lSTEPS,{'structural_normalize','structural_normalize_preservemasks','structural_segment&normalize','structural_segment&normalize_withlesion','functional_segment&normalize_indirect_withlesion','structural_segment','functional_normalize','functional_segment&normalize','functional_segment','functional_segment&normalize_direct','functional_segment&normalize_indirect','functional_normalize_indirect','functional_normalize_indirect_preservemasks','functional_normalize_direct','structural_manualspatialdef','functional_manualspatialdef'}))
    ...
    ht0=uicontrol('style','edit','units','norm','position',[.7,.35,.2,.1],'string',mat2str(boundingbox),'tooltipstring',conn_menu_formathtml('<HTML>defines bounding box of resampled volumes<br/> - enter a 2x3 matrix with minimum xyz values in the top row and maximum xyz values in the bottom row</HTML>'));
    if ~all(ismember(lSTEPS,{'structural_manualspatialdef','functional_manualspatialdef'})), 
        ...
        if ~ok, val=1; end
        ht7=uicontrol('style','popupmenu','units','norm','position',[.7,.25,.2,.1],'string',{'mni','eastern','rigid','subj','no regularization','no coregistration'},'value',val,'tooltipstring',conn_menu_formathtml('<HTML>defines affine-coregistration regularization options<br/> - choose ''mni'' for registration of European brains with MNI space<br/> - choose ''eastern'' for registration of East Asian brains with MNI space<br/> - choose ''rigid'' for rigid(ish)-body registration<br/> - choose ''subj'' for inter-subject registration<br/> - choose ''no regularisation'' for unregularized affine coregistration<br/> - choose ''no coregistration'' to skip affine coregistration step<br/> - see "help spm_affine_priors" for details-</HTML>'));
    else ht7=[];
    end
    uicontrol('style','pushbutton','string','OK','units','norm','position',[.1,.01,.38,.15],'callback','uiresume');
    uicontrol('style','pushbutton','string','Cancel','units','norm','position',[.51,.01,.38,.15],'callback','delete(gcbf)');
...
if any(cellfun('length',regexp(lSTEPS,'^functional_label_as_|^structural_label_as_'))&~ismember(lSTEPS,{'functional_label_as_original', 'functional_label_as_subjectspace', 'functional_label_as_minprep', 'functional_label_as_realigned', 'functional_label_as_mnispace', 'functional_label_as_surfacespace', 'functional_label_as_smoothed','structural_label_as_original', 'structural_label_as_mnispace','structural_label_as_subjectspace'}))
...
if any(cellfun('length',regexp(lSTEPS,'^functional_load_from_|^structural_load_from_'))&~ismember(lSTEPS,{'functional_load_from_original', 'functional_load_from_subjectspace', 'functional_load_from_minprep', 'functional_load_from_realigned', 'functional_load_from_mnispace', 'functional_load_from_surfacespace', 'functional_load_from_smoothed','structural_load_from_original', 'structural_load_from_mnispace','structural_load_from_subjectspace'}))
...
for iSTEP=1:numel(STEPS)
    ...
    switch(regexprep(lower(STEP),'^run_|^update_|^interactive_',''))
        ...
        case 'functional_sliceintensity'
            for isubject=1:numel(subjects),
                nsubject=subjects(isubject);
                nsess=CONN_x.Setup.nsessions(min(numel(CONN_x.Setup.nsessions),nsubject));
                for nses=1:nsess,
        ...
        case 'functional_roiextract'
            if isempty(bp_filter)&&any(roi_filter), error('Band-pass filter not specified (missing #bp_filter field)'); end
            conn_disp('fprintf','extraction of ROI timeseries (%s)\n',sprintf('%s ',roi_names{:}));
            conn_disp('fprintf','dimensions = %s; derivatives = %s; filtered = %s; detrend = %d; PSC scale = %d; orthogonal 1st-level covariates = [%s]\n',mat2str(roi_dimensions),mat2str(roi_deriv),mat2str(roi_filter),roi_detrend,roi_scale,sprintf('%s ',roi_names{ismember(roi_names,CONN_x.Setup.l1covariates.names(1:end-1))}));
            for isubject=1:numel(subjects),
                nsubject=subjects(isubject);
                nsess=CONN_x.Setup.nsessions(min(numel(CONN_x.Setup.nsessions),nsubject));
                for nses=1:nsess,
                    if ismember(nses,sessions)
                        ...
                        filein=cellstr(filename);
                        Vin=spm_vol(char(filein));
                        if any(roi_filter), rt=conn_get_rt(nsubject,nses,sets); end
                        entercovariates=[];
                        if roi_detrend, entercovariates=[entercovariates,linspace(-1,1,numel(Vin))']; end
                        for nl1covariate=1:numel(roi_names)
                            icov=find(strcmp(CONN_x.Setup.l1covariates.names(1:end-1),roi_names{nl1covariate}));
                            if ~isempty(icov) % first-level covariates
                                cfilename=CONN_x.Setup.l1covariates.files{nsubject}{icov}{nses}{1};
                                assert(~isempty(cfilename),'covariate %s has not been defined for subject %d sessions %d',roi_names{nl1covariate},nsubject,nses);
                                switch(cfilename),
                                    case '[raw values]',
                                        data=CONN_x.Setup.l1covariates.files{nsubject}{icov}{nses}{3};
                                    otherwise,
                                ...
                                if numel(roi_dimensions)>=nl1covariate, data=data(:,1:min(size(data,2),roi_dimensions(nl1covariate))); end
                                entercovariates=cat(2,entercovariates,data-repmat(mean(data,1),size(data,1),1)); % note: same as ROI entercovariates behavior
                            end
                        end
                        ...
                        for nl1covariate=1:numel(roi_names)
                            ...
                            if isempty(icov) % ROIs
                                ...
                                assert(size(data,1)==numel(Vin),'mismatched dimensions; functional data has %d timepoints, covariate %s has %d timepoints',numel(Vin),roi_names{nl1covariate},size(data,1));
                                if numel(roi_dimensions)>=nl1covariate, data=data(:,1:min(size(data,2),roi_dimensions(nl1covariate))); end
                                if ~isempty(entercovariates), data = data - entercovariates*(pinv(entercovariates'*entercovariates)*entercovariates'*data); end % orthogonalize to covariates
        ...
        case 'functional_centerruns'
        case 'functional_centertostruct'
        case 'structural_center'
        case {'functional_label','functional_label_as_original', 'functional_label_as_subjectspace', 'functional_label_as_minprep', 'functional_label_as_realigned', 'functional_label_as_mnispace', 'functional_label_as_surfacespace', 'functional_label_as_smoothed'}
        case {'functional_load','functional_load_from_original', 'functional_load_from_subjectspace', 'functional_load_from_minprep', 'functional_load_from_realigned', 'functional_load_from_mnispace', 'functional_load_from_surfacespace', 'functional_load_from_smoothed'}
        case {'structural_label','structural_label_as_original', 'structural_label_as_mnispace','structural_label_as_subjectspace'}
        case {'structural_load','structural_load_from_original', 'structural_load_from_mnispace','structural_load_from_subjectspace'}
                        ...
                        while (numel(unique(sliceorder))~=nslice||max(sliceorder)~=nslice||min(sliceorder)~=1) && (numel(sliceorder)~=nslice||any(sliceorder<0|sliceorder>conn_get_rt(nsubject,nses,sets)*1000))
                            ...
                            switch(sliceorder_select)
                                ...
                                case 8, % BIDS json file
                                    ...
                                    if isempty(str), 
                                        str=conn_jsonread(matlabbatch{end}.spm.temporal.st.scans{1}{1},'SliceOrder');
                                        if isempty(str), conn_disp('fprintf','ERROR: SliceTiming information not found in json file associated with %s (new user-input required)\n',matlabbatch{end}.spm.temporal.st.scans{1}{1});
                                            ...
                                            switch(lower(char(str))) %'ascending','descending','interleaved (middle-top)','interleaved (bottom-up)','interleaved (top-down)','interleaved (Siemens)','interleaved (Philips)'
                                                case 'ascending', sliceorder=1:nslice;        % ascending
                                                case 'descending', sliceorder=nslice:-1:1;     % descending
                                                case 'interleaved (middle-top)', sliceorder=round((nslice-(1:nslice))/2 + (rem((nslice-(1:nslice)),2) * (nslice - 1)/2)) + 1; % interleaved (middle-top)
                                                case 'interleaved (bottom-up)', sliceorder=[1:2:nslice 2:2:nslice]; % interleaved (bottom-up)
                                                case 'interleaved (top-down)', sliceorder=[nslice:-2:1, nslice-1:-2:1]; % interleaved (top-down)
                                                case 'interleaved (siemens)', sliceorder=[fliplr(nslice:-2:1) fliplr(nslice-1:-2:1)]; % interleaved (Siemens)
                                                case 'interleaved (philips)', sliceorder=cell2mat(arrayfun(@(n)n:round(sqrt(nslice)):nslice,1:round(sqrt(nslice)),'uni',0)); % interleaved (Philips)
                                                otherwise, conn_disp('fprintf','ERROR: unrecognized SliceOrder information in json file associated with %s (valid values are ascending,descending,interleaved (middle-top),interleaved (bottom-up),interleaved (top-down),interleaved (Siemens),interleaved (Philips); new user-input required)\n',matlabbatch{end}.spm.temporal.st.scans{1}{1});
                                            end
                                        end
            ...
            for isubject=1:numel(subjects),
                ...
                for nses_struct=nsess_struct(:)'
                    ...
                    if ~isempty(nsess_func)
                        ...
                        newxtemp=conn_prepend('r',xtemp);
                        if coregtomean, 
                            conn_fileutils('filecopy',xtemp,newxtemp); 
                            if any(conn_existfile(conn_prepend('',xtemp,'.mat'))), conn_fileutils('filecopy',conn_prepend('',xtemp,'.mat'),conn_prepend('',newxtemp,'.mat')); end
                        end
                        xtemp=newxtemp;
                        matlabbatch{end+1}.spm.spatial.coreg.estimate.source=xtemp;
                        if isempty(CONN_x.Setup.structural{nsubject}{nses_struct}{1}), error(sprintf('No structural file defined for Subject %d Session %d. Please select a structural file',nsubject,nses_struct));
                        ...
                        for nsestrue=nsess_func(:)'
                            ...
                            if any(conn_existfile(conn_prepend('',temp,'.mat'))), conn_fileutils('filecopy',conn_prepend('',temp,'.mat'),conn_prepend('',newtemp,'.mat')); end
                            temp=newtemp; 
                            if numel(temp)==1, ttemp=cellstr(conn_expandframe(temp{1})); else ttemp=temp; end
                            matlabbatch{end}.spm.spatial.coreg.estimate.other=cat(1,matlabbatch{end}.spm.spatial.coreg.estimate.other,ttemp);
                            outputfiles{isubject}{nsestrue}=char(temp);
                        end
                    end
                end
            end
    ...
    if strncmp(lower(STEP),'interactive_',numel('interactive_'))
        ...
        elseif any(strcmpi(regexprep(lower(STEP),'^run_|^update_|^interactive_',''),{'functional_removescans','functional_bandpass','functional_regression','functional_sliceintensity','functional_roiextract','functional_mask','structural_mask','functional_manualorient','structural_manualorient','functional_center','functional_centerruns','functional_centertostruct','structural_center','functional_motionmask','functional_label','functional_label_as_original', 'functional_label_as_subjectspace', 'functional_label_as_minprep', 'functional_label_as_realigned', 'functional_label_as_mnispace', 'functional_label_as_surfacespace', 'functional_label_as_smoothed','functional_load','functional_load_from_original', 'functional_load_from_subjectspace', 'functional_load_from_minprep', 'functional_load_from_realigned', 'functional_load_from_mnispace', 'functional_load_from_surfacespace', 'functional_load_from_smoothed','structural_label','structural_label_as_original', 'structural_label_as_mnispace', 'structural_label_as_subjectspace', 'structural_load','structural_load_from_original', 'structural_load_from_mnispace', 'structural_load_from_subjectspace', 'functional_surface_smooth','functional_surface_resample'}))
    ...
    if ok>=0&&doimport
        ...
        switch(regexprep(lower(STEP),'^run_|^update_|^interactive_',''))
            ...
            case {'functional_bandpass','functional_sliceintensity'}
                for isubject=1:numel(subjects),
                    nsubject=subjects(isubject);
                    for nses=1:numel(outputfiles{isubject})
                        if ismember(nses,sessions)
            ...
            case 'functional_roiextract'
                if ~sets||ALLSETSPERMISSIONS,
                    icov=find(strcmp(CONN_x.Setup.l1covariates.names(1:end-1),'QC_rois'));
                    if isempty(icov),
                        icov=numel(CONN_x.Setup.l1covariates.names);
                        CONN_x.Setup.l1covariates.names{icov}='QC_rois';
                        CONN_x.Setup.l1covariates.names{icov+1}=' ';
                    end
                    for isubject=1:numel(subjects),
                        nsubject=subjects(isubject);
            ...
            case 'functional_label_as_minprep'
                conn_datasetcopy(sets,'minimally-preprocessed functional data',subjects);
            ...
            case 'functional_load_from_minprep'
                if isempty(conn_datasetlabel('minimally-preprocessed functional data'))&&~isempty(conn_datasetlabel('minimally-preprocessed data')), conn_datasetcopy('minimally-preprocessed data',sets,subjects); % note: back-compatibility
                else conn_datasetcopy('minimally-preprocessed functional data',sets,subjects);
                end
            ...
            case 'structural_label_as_subjectspace'
                conn_datasetcopy(-1,'subject-space structural images',subjects);
            ...
            case 'structural_load_from_subjectspace'
                conn_datasetcopy('subject-space structural images',-1,subjects);
            ...
            case 'functional_centerruns'
                ...
                for isubject=1:numel(subjects),
                    ...
                    translation=[];
                    for nses=1:nsess
                        if 1, translation=[]; end % center each run/session separately
                        if ismember(nses,sessions)
                            [filename{nsubject}{nses},cfile]=conn_get_functional(nsubject,nses,sets,[],'cfile');
                            if isempty(filename{nsubject}{nses}), error('Functional data not yet defined for subject %d session %d',nsubject,nses); end
                            temp=cellstr(filename{nsubject}{nses});
                            ...
                            if 0,%coregtomean, % keeps mean image in same space in case it is required later
                                [xtemp,failed]=conn_setup_preproc_meanimage(temp{1});
                                if ~isempty(xtemp), temp=[{xtemp};temp]; end
                            end
                            M=cell(1,numel(temp));
                        ...
                        if ismember(nses,sessions)
                            ...
                            for n=1:numel(temp)
                                ...
                                if isempty(translation)
                                    ...
                                    conn_disp('fprintf','Functional centering translation x/y/z = %s (Subject %d)\n',mat2str(translation'),nsubject);
                                    try, R=eye(4);R(1:3,4)=translation; save(conn_prepend('centeringtostruct_',temp{n},'.mat'),'R','-mat'); R=inv(R); save(conn_prepend('icenteringtostruct_',temp{n},'.mat'),'R','-mat'); end
                                end
                                M{n}(1:3,4)=M{n}(1:3,4)+translation;
                            end
                            for n=1:numel(temp)
