function [dataA,labelA,infoA,lineA,fileout]=conn_qaplots_covupdate(X,Xnames,Xdescr,nsubs,qafolder,ht,ht_offset,ht_scale,dirX)
...
if nargin<9||isempty(dirX), dirX=[]; end
...
py=zeros([Npts,size(Xdisp)]); for ny=1:size(Xdisp,2), py(:,:,ny)=exp(-.5*(repmat(hx,1,size(Xdisp,1))-repmat(Xdisp(:,ny)',Npts,1)).^2/(.05/kt).^2); end
ny=sum(~isnan(py),2); %
py(isnan(py))=0;
hy=cumsum(py,2);
...
dataA={};labelA={};infoA={};lineA={};fileout={};
for isub=1:numel(nsubs)
    ...
    if ~isempty(dirX)
        outlierscore=nan(1,size(IQ,2));
        outlierscore(dirX==0)=max(((IQ(2,dirX==0)-Xthis(dirX==0))), ((Xthis(dirX==0))-IQ(4,dirX==0)))./(IQ(4,dirX==0)-IQ(2,dirX==0));
        outlierscore(dirX>0)=(Xthis(dirX>0)-IQ(4,dirX>0))./(IQ(4,dirX>0)-IQ(2,dirX>0));
        outlierscore(dirX<0)=(IQ(2,dirX<0)-Xthis(dirX<0))./(IQ(4,dirX<0)-IQ(2,dirX<0));
        results_info=struct('Values',Xthis,'ValuesDisplay',Xdispthis,'Interquartiles',IQ,'InterquartilesDisplay',IQdisp,'OutlierScores',outlierscore,'Subjects',{nsub},'Variables',{Xnames},'Variables_descr',{Xdescr},'Variables_dir',dirX);
    else
        results_info=struct('Values',Xthis,'ValuesDisplay',Xdispthis,'Interquartiles',IQ,'InterquartilesDisplay',IQdisp,'Subjects',{nsub},'Variables',{Xnames},'Variables_descr',{Xdescr});
    end
