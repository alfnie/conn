...
%         else, nsources=varargin{1}; data.source=0; end
        ...
        data.Zthr=.15; %05:.01:.30; % note: switch to cost-integration default between 5% and 30% (change to .15 for reproducibility of prior results);
        data.Zthrtype=3;
        data.Zthrside=1;
        data.thr=.05;
        data.thrtype=2;
        ...
        data.handles=[...
            ...
            uicontrol('style','popupmenu','units','norm','position',[.87,.78,.11,.04],'string',{'one-sided (positive)','one-sided (negative)','two-sided'},'foregroundcolor','k','backgroundcolor',color2,'callback',{@conn_displaynetwork,'side'},'value',data.side,'tooltipstring','Second-level analysis results directionality'),...
            ...
            uicontrol('style','edit','units','norm','position',[.70,.90,.05,.04],'string',num2str(data.Zthr),'foregroundcolor','k','backgroundcolor',color2,'callback',{@conn_displaynetwork,'Zthr'},'tooltipstring',conn_menu_formathtml('<HTML>Edge-defining threshold value (two nodes are connected by an edge if their connectivity strength is above this value)<br/> - enter multiple thresholds to use cost-integration (averaging of graph measures across multiple networks, each defined by a different threshold)<br/> - use a:b:c syntax to specif arbitrary ranges (from a to c in steps of size b)</HTML>')),...
...
switch(data.display),
    case 'connectivity',
        ...
        tp=[];sort2=[];txt2={};
        if 1,
            txt2{end+1}=conn_menu_formathtml(sprintf('<HTML><pre><b>%-20s %6.2f  %6.2f  %4d  %8.6f</b></pre></HTML>',['NETWORK'],data.h_net(data.displaymeasure),data.F_net(data.displaymeasure),data.dof_net(data.displaymeasure,end),p_net));
        end
        for n1=1:N,
            if n1<=N&&z(n1)>0,
