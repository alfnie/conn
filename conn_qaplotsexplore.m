...
dlg.handles.analysis=uicontrol('style','popupmenu','units','norm','position',[.125,.835,.5,.05],'string',' ','backgroundcolor',figcolor,'foregroundcolor','k','tooltipstring',conn_menu_formathtml('<HTML>Select a Quality Assurance plot within this report<br/> - each report may contain one or multiple plots<br/> - choose one existing plot from this report, or select <i>new plot</i> to create a new plot and add it to this report</HTML>'),'callback',{@conn_qaplotsexplore_update,'plot'},'fontsize',9+font_offset,'interruptible','off');
...
dlg.handles.showdiff=uicontrol('style','checkbox','units','norm','position',[.02 .07 .15 .05],'string','show diff','backgroundcolor',figcolor,'foregroundcolor','k','fontsize',9+font_offset,'tooltipstring',conn_menu_formathtml('<HTML>Show z score maps: normalized differences between each image and the average of all images in this plot<br/>this can be useful to highlight differences between a selected subject and the group</HTML>'),'value',0,'callback',{@conn_qaplotsexplore_update,'subjects'},'interruptible','off');
...
%                 tfilename=conn_prepend('',fullfile(outputDir,'report.html'),regexprep(options{strmatch(answ,options(:,2),'exact'),1},'*',''));
                ...
                if strcmp(format,'html')&&~isempty(which('private/mxdom2simplehtml.xsl')),
                    ...
                    linebreak=sprintf('%s\n',['hfig=figure(''units'',''norm'',''position'',[.3 .4 .4 .2],''color'',',mat2str(figcolor),'); plot([-1 1],[0 0],''k-'',''linewidth'',2); set(gca,''xlim'',[-1 1],''ylim'',[-1 1]); axis off; snapnow; close(hfig);']);
                else
                    stylesheet={};
                    linebreak=sprintf('%s\n','hfig=figure(''units'',''norm'',''position'',[.3 .4 .4 .2],''color'',''w''); plot([-1 1],[0 0],''k-'',''linewidth'',2); set(gca,''xlim'',[-1 1],''ylim'',[-1 1]); axis off; snapnow; close(hfig);');
                end
                %
                ...
                for np=1:numel(dlg.uanalyses)
                    ...
                    if ismember(np,plots_summary)
                        ...
                        tfh{end+1}=sprintf('options(''togglegui'',''on'');\n');
                    ...
                    for np=plots_subject(:)'
                        ...
                        if numel(usubjects_shown)>1||(numel(usubjects_shown)==1&&numel(umeasures_shown)==1)
                            ...
                            for ns=1:numel(usubjects_shown)
                                ...
                                usessions_shown=unique(dlg.isessions(in(dlg.isubjects(in)==usubjects_shown(ns))));
                                umeasures_shown=unique(dlg.imeasures(in(dlg.isubjects(in)==usubjects_shown(ns))));
                                if numel(umeasures_shown)>1, tfh{end+1}=sprintf('options(''preselectall'');\n'); end
                                tfh{end+1}=sprintf('options(''selectsubjects'',%d);\n',ns);
                                tfh{end+1}=sprintf('options(''displayannotation'');\n');
                                for nses=1:numel(usessions_shown)
                                    tfh{end+1}=sprintf('options(''selectsessions'',%d);\n',nses);
                                    tfh{end+1}=sprintf('options(''togglegui'',''on'');\n');
                                    tfh{end+1}=sprintf('snapnow;\n');
                                end
                            end
            ...
            case 'togglegui'
                ...
                elseif ~isfield(dlg,'togglegui')||isempty(dlg.togglegui), onoff=1;
                else onoff=2;
                end                   
                if onoff==1  % on (hides gui)
            ...
            case 'annotate'
                ...
                %fclose(fh);
            case 'figurevisible',
                set(dlg.handles.hfig,'visible',varargin{1});
            case 'invertimage',
            ...
            case {'newplot','recreateplot','recreateset'}
                ...
                defaultset=[11,13,31]; %[1,2,4,5,7,9,11,12,13,31];
                ...
                h4d=uicontrol('units','norm','position',[.1,.25,.25,.30],'style','listbox','max',2,'string',arrayfun(@(n)sprintf('subject %d',n),1:CONN_x.Setup.nsubjects,'uni',0),'value',1:CONN_x.Setup.nsubjects,'tooltipstring',conn_menu_formathtml('<HTML>Select subjet(s) to include in these plots<br/> - right-click for additional options</HTML>'));
                ...
                h4a=uicontrol('units','norm','position',[.45,.47,.45,.08],'style','listbox','max',2,'string',[{'functional data'},arrayfun(@(n)sprintf('secondary dataset #%d %s',n,regexprep(CONN_x.Setup.secondarydataset(n).label,'(.+)','($1)')),1:numel(CONN_x.Setup.secondarydataset),'uni',0)],'value',[],'tooltipstring',conn_menu_formathtml('<HTML>Select functional dataset(s) to include in functional data plots</HTML>'));
                h4b=uicontrol('units','norm','position',[.45,.38,.45,.08],'style','listbox','max',2,'string',[CONN_x.Setup.rois.names(1:end-1), regexprep(CONN_x.Setup.rois.names(1:3),'^(.*)$','eroded $1')],'value',[],'tooltipstring',conn_menu_formathtml('<HTML>Select ROI(s) to include in ROI data plots</HTML>'));
                h4c=uicontrol('units','norm','position',[.45,.29,.45,.08],'style','listbox','max',2,'string',nl2covariates,'value',[],'tooltipstring',conn_menu_formathtml('<HTML>Select QC variable(s) to include in QC-FC plots</HTML>'));
                h4e=uicontrol('units','norm','position',[.45,.20,.45,.08],'style','listbox','max',2,'string',CONN_x.Setup.conditions.names(validconditions),'value',[],'tooltipstring',conn_menu_formathtml('<HTML>QC/FC plots aggregate FC data across multiple runs/sessions <br/> Select individual conditions if you want to only aggregate in these plots <br/> across runs/sessions where at least one of the selected conditions is present</HTML>'));
                ...
                if ishandle(fh),
                    ...
                    if nalt==1,
                        ...
                        if numel(validsubjects)>1
                            answer=conn_menu_inputdlg('Number of parallel processes (run simultaneously)?','CONN HPC',1,{'1'});
                            if isempty(answer), return; end
                            N=str2num(answer{1});
                        else N=1;
                        end
            ...
            case 'set'
                ...
                set(dlg.handles.analysis,'string',conn_menu_formathtml([{'<HTML><i>choose an existing QA plot to display</i></HTML>'},dlg.uanalyses_long]),'value',dlg.ianalysis+1);
                ...
                switch(dlg.uanalysestype(dlg.ianalysis))
                    ...
                    case {2,3,4} % QA_DENOISE
                            ...
                            if ~isempty(dlg.results_info)&&isstruct(dlg.results_info{1}),
                                if isfield(dlg.results_info{1},'IntersectionBefore')
                                    if isfield(dlg.results_info{1},'PercentSignificantBefore')
                                        text(-.95,dlg.plothistinfo2*.25-min(dlg.plothistinfo2,dlg.plothistinfo3-dlg.plothistinfo2)*.15,{sprintf('%.1f%% overlap coefficient (mean=%.2f sd=%.2f)',100*dlg.results_info{1}.IntersectionAfter,dlg.results_info{1}.MeanAfter,dlg.results_info{1}.StdAfter),sprintf('%.1f%% edges with p<.05, %.1f%% edges with q<.05',100*dlg.results_info{1}.PercentSignificantAfter(1),100*dlg.results_info{1}.PercentSignificantAfter(2))},'horizontalalignment','left','fontsize',7+font_offset,'parent',dlg.handles.hax);
                                        text(-.95,dlg.plothistinfo2+(dlg.plothistinfo3-dlg.plothistinfo2)*.25-min(dlg.plothistinfo2,dlg.plothistinfo3-dlg.plothistinfo2)*.15,{sprintf('%.1f%% overlap coefficient (mean=%.2f sd=%.2f)',100*dlg.results_info{1}.IntersectionBefore,dlg.results_info{1}.MeanBefore,dlg.results_info{1}.StdBefore),sprintf('%.1f%% edges with p<.05, %.1f%% edges with q<.05',100*dlg.results_info{1}.PercentSignificantBefore(1),100*dlg.results_info{1}.PercentSignificantBefore(2))},'horizontalalignment','left','fontsize',7+font_offset,'parent',dlg.handles.hax);
                                    else
                                        text(-.95,dlg.plothistinfo2*.25-min(dlg.plothistinfo2,dlg.plothistinfo3-dlg.plothistinfo2)*.15,{sprintf('mean=%.2f sd=%.2f (%.1f%% overlap coefficient)',dlg.results_info{1}.MeanAfter,dlg.results_info{1}.StdAfter,100*dlg.results_info{1}.IntersectionAfter)},'horizontalalignment','left','fontsize',7+font_offset,'parent',dlg.handles.hax);
                                        text(-.95,dlg.plothistinfo2+(dlg.plothistinfo3-dlg.plothistinfo2)*.25-min(dlg.plothistinfo2,dlg.plothistinfo3-dlg.plothistinfo2)*.15,{sprintf('mean=%.2f sd=%.2f (%.1f%% overlap coefficient)',dlg.results_info{1}.MeanBefore,dlg.results_info{1}.StdBefore,100*dlg.results_info{1}.IntersectionBefore)},'horizontalalignment','left','fontsize',7+font_offset,'parent',dlg.handles.hax);
                                    end
                                elseif isfield(dlg.results_info{1},'DofBefore')&&isfield(dlg.results_info{1},'PeakBefore')
                                    text(-.95,dlg.plothistinfo2*.25-min(dlg.plothistinfo2,dlg.plothistinfo3-dlg.plothistinfo2)*.15,{sprintf('peak r=%.2f sd=%.2f df=%.1f',dlg.results_info{1}.PeakAfter,dlg.results_info{1}.StdAfter,dlg.results_info{1}.DofAfter)},'horizontalalignment','left','fontsize',7+font_offset,'parent',dlg.handles.hax);
                                    text(-.95,dlg.plothistinfo2+(dlg.plothistinfo3-dlg.plothistinfo2)*.25-min(dlg.plothistinfo2,dlg.plothistinfo3-dlg.plothistinfo2)*.15,{sprintf('peak r=%.2f sd=%.2f df=%.1f',dlg.results_info{1}.PeakBefore,dlg.results_info{1}.StdBefore,dlg.results_info{1}.DofBefore)},'horizontalalignment','left','fontsize',7+font_offset,'parent',dlg.handles.hax);
                                elseif isfield(dlg.results_info{1},'DofBefore')
                                    text(-.95,dlg.plothistinfo2*.25-min(dlg.plothistinfo2,dlg.plothistinfo3-dlg.plothistinfo2)*.15,{sprintf('mean=%.2f sd=%.2f df=%.1f',dlg.results_info{1}.MeanAfter,dlg.results_info{1}.StdAfter,dlg.results_info{1}.DofAfter)},'horizontalalignment','left','fontsize',5+font_offset,'parent',dlg.handles.hax);
                                    text(-.95,dlg.plothistinfo2+(dlg.plothistinfo3-dlg.plothistinfo2)*.25-min(dlg.plothistinfo2,dlg.plothistinfo3-dlg.plothistinfo2)*.15,{sprintf('mean=%.2f sd=%.2f df=%.1f',dlg.results_info{1}.MeanBefore,dlg.results_info{1}.StdBefore,dlg.results_info{1}.DofBefore)},'horizontalalignment','left','fontsize',5+font_offset,'parent',dlg.handles.hax);
                                elseif isfield(dlg.results_info{1},'CorrBefore')
                                    ...
                                    text(-.95,dlg.plothistinfo2*.25-min(dlg.plothistinfo2,dlg.plothistinfo3-dlg.plothistinfo2)*.15,{sprintf('mean=%.2f sd=%.2f',dlg.results_info{1}.MeanAfter,dlg.results_info{1}.StdAfter)},'horizontalalignment','left','fontsize',5+font_offset,'parent',dlg.handles.hax);
                                    text(-.95,dlg.plothistinfo2+(dlg.plothistinfo3-dlg.plothistinfo2)*.25-min(dlg.plothistinfo2,dlg.plothistinfo3-dlg.plothistinfo2)*.15,{sprintf('mean=%.2f sd=%.2f',dlg.results_info{1}.MeanBefore,dlg.results_info{1}.StdBefore)},'horizontalalignment','left','fontsize',5+font_offset,'parent',dlg.handles.hax);
                                end
                            end
                        ...
                        elseif ~isempty(dlg.results_info)&&isstruct(dlg.results_info{1})&&isfield(dlg.results_info{1},'IntersectionBefore')&&isfield(dlg.results_info{1},'PercentSignificantBefore')
                            text(-.95,dlg.plothistinfo2*.25-min(dlg.plothistinfo2,dlg.plothistinfo3-dlg.plothistinfo2)*.15,{sprintf('%.1f%% overlap coefficient (combined %d measures)',100*min(cellfun(@(x)x.IntersectionAfter,dlg.results_info)),numel(in))},'horizontalalignment','left','fontsize',8+font_offset,'parent',dlg.handles.hax);
                            text(-.95,dlg.plothistinfo2+(dlg.plothistinfo3-dlg.plothistinfo2)*.25-min(dlg.plothistinfo2,dlg.plothistinfo3-dlg.plothistinfo2)*.15,{sprintf('%.1f%% overlap coefficient (combined %d measures)',100*min(cellfun(@(x)x.IntersectionBefore,dlg.results_info)),numel(in))},'horizontalalignment','left','fontsize',8+font_offset,'parent',dlg.handles.hax);
                            %text(-.95,dlg.plothistinfo2*.25-min(dlg.plothistinfo2,dlg.plothistinfo3-dlg.plothistinfo2)*.15,{sprintf('combined %.1f%% overlap coefficient',100*sum(minshow1)),sprintf('mean=%.2f std=%.2f',mean(cellfun(@(x)x.MeanAfter,dlg.results_info)),mean(cellfun(@(x)x.StdAfter,dlg.results_info)))},'horizontalalignment','left','fontsize',5+font_offset,'parent',dlg.handles.hax);
                            %text(-.95,dlg.plothistinfo2+(dlg.plothistinfo3-dlg.plothistinfo2)*.25-min(dlg.plothistinfo2,dlg.plothistinfo3-dlg.plothistinfo2)*.15,{sprintf('combined %.1f%% overlap coefficient',100*sum(minshow2)),sprintf('mean=%.2f sd=%.2f',mean(cellfun(@(x)x.MeanBefore,dlg.results_info)),mean(cellfun(@(x)x.StdBefore,dlg.results_info)))},'horizontalalignment','left','fontsize',5+font_offset,'parent',dlg.handles.hax);
                        elseif ~isempty(dlg.results_info)&&isfield(dlg.results_info{1},'PeakBefore')&&(isfield(dlg.results_info{1},'DofBefore')||isfield(dlg.results_info{1},'MeanBefore'))
                            text(-.95,dlg.plothistinfo2*.25-min(dlg.plothistinfo2,dlg.plothistinfo3-dlg.plothistinfo2)*.15,{sprintf('peak r=%.3f%c%.3f sd=%.3f%c%.3f',mean(cellfun(@(x)x.PeakAfter,dlg.results_info)),177,std(cellfun(@(x)x.PeakAfter,dlg.results_info)),mean(cellfun(@(x)x.StdAfter,dlg.results_info)),177,std(cellfun(@(x)x.StdAfter,dlg.results_info)))},'horizontalalignment','left','fontsize',8+font_offset,'parent',dlg.handles.hax);
                            text(-.95,dlg.plothistinfo2+(dlg.plothistinfo3-dlg.plothistinfo2)*.25-min(dlg.plothistinfo2,dlg.plothistinfo3-dlg.plothistinfo2)*.15,{sprintf('peak r=%.3f%c%.3f sd=%.3f%c%.3f',mean(cellfun(@(x)x.PeakBefore,dlg.results_info)),177,std(cellfun(@(x)x.PeakBefore,dlg.results_info)),mean(cellfun(@(x)x.StdBefore,dlg.results_info)),177,std(cellfun(@(x)x.StdBefore,dlg.results_info)))},'horizontalalignment','left','fontsize',8+font_offset,'parent',dlg.handles.hax);
                        ...
                        elseif ~isempty(dlg.results_info)&&isfield(dlg.results_info{1},'MeanBefore')
                            text(-.95,dlg.plothistinfo2*.25-min(dlg.plothistinfo2,dlg.plothistinfo3-dlg.plothistinfo2)*.15,{sprintf('mean=%.2f sd=%.2f',mean(cellfun(@(x)x.MeanAfter,dlg.results_info)),mean(cellfun(@(x)x.StdAfter,dlg.results_info)))},'horizontalalignment','left','fontsize',5+font_offset,'parent',dlg.handles.hax);
                            text(-.95,dlg.plothistinfo2+(dlg.plothistinfo3-dlg.plothistinfo2)*.25-min(dlg.plothistinfo2,dlg.plothistinfo3-dlg.plothistinfo2)*.15,{sprintf('mean=%.2f sd=%.2f',mean(cellfun(@(x)x.MeanBefore,dlg.results_info)),mean(cellfun(@(x)x.StdBefore,dlg.results_info)))},'horizontalalignment','left','fontsize',5+font_offset,'parent',dlg.handles.hax);
                        end
                        if dlg.uanalysestype(dlg.ianalysis)==4,
                    ...
                    case 5 % QA_COV
                        ...
                        dlg.handles.resultsline=[];
                        dlg.handles.resultslineoutlier=[];
                        for n=1:numel(dlg.results_patch),
                            ...
                            if isfield(dlg.results_info{n},'OutlierScores')
                                plot(dlg.results_line{n}{1}(dlg.results_info{n}.OutlierScores>3),dlg.results_line{n}{2}(dlg.results_info{n}.OutlierScores>3),'rx','markersize',10+font_offset,'parent',dlg.handles.hax);
                            end
                        end
                        if numel(in)==1,
