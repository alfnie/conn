                    ...
                    for np=plots_subject(:)'
                        ...
                        if numel(usubjects_shown)>1||(numel(usubjects_shown)==1&&numel(umeasures_shown)==1)
                            for ns=1:numel(usubjects_shown)
                                ...
                                tfh{end+1}=sprintf('%% <matlab:options=conn_qaplotsexplore(''%s'');options(''plot'',%d);options(''preselectall'');options(''selectsubjects'',%d); [open plot in Matlab]>\n',regexprep(outputDir,' ','%20'),np,ns);
                                if ns==1,
                                    tfh{end+1}=sprintf('options=conn_qaplotsexplore(''%s'');\n',regexprep(outputDir,' ','%20'));
                                    if invertim, tfh{end+1}=sprintf('options(''invertimage'',0);\n'); end
                                    tfh{end+1}=sprintf('options(''plot'',%d);\n',np);
                        ...
                        elseif numel(umeasures_shown)>1
                            for ns=1:numel(umeasures_shown)
                                ...
                                tfh{end+1}=sprintf('%% <matlab:options=conn_qaplotsexplore(''%s'');options(''plot'',%d);options(''preselectall'');options(''selectmeasures'',%d); [open plot in Matlab]>\n',regexprep(outputDir,' ','%20'),np,ns);
                                if ns==1,
                                    tfh{end+1}=sprintf('options=conn_qaplotsexplore(''%s'');\n',regexprep(outputDir,' ','%20'));
                                    if invertim, tfh{end+1}=sprintf('options(''invertimage'',0);\n'); end
                                    tfh{end+1}=sprintf('options(''plot'',%d);\n',np);
